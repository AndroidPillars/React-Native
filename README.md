...
you typically build a React Native app by mostly writting Javascript code or depending on the app you're building, by entirely writing Javascript. Now I say mostly, atleast that's a possibility because you can also write native code for iOS and Android and React Native 
gives you the tools to connect your Javascript code to that native code, though that's a bit more advanced and in many apps,You will 
never need that and therefore, React Native gives you full flexibility, it gives you a way of connecting Javascript to native code and 
it also gives you a lot of pre-built native features which are conveniently usable from inside your Javascript code. So if you then 
combine ReactNative and the features that gives you with React,js which knows how to update a user interface and how to control user 
interface, themn you get everything you need tocompile a real native mobile app and that's also what React Native gives you, it gives 
